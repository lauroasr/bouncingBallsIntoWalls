...
Util.getRandomInteger = function (minimum, maximum) {
	return Math.round(Math.random() * (maximum - minimum) + minimum);
};

Util.getNumberTransition = function (minimum, maximum, ratio) {
...
Util.Color.prototype.getTransition = function (finalColor, ratio) {
	var red = Util.getNumberTransition(this.red, finalColor.red, ratio);
	var green = Util.getNumberTransition(this.green, finalColor.green, ratio);
	var blue = Util.getNumberTransition(this.blue, finalColor.blue, ratio);
	var alpha = Util.getNumberTransition(this.alpha, finalColor.alpha, ratio);
...
Util.Vector.prototype.getPathTo = function (other) {
...
/* auto setters */
Util.Vector.prototype.updateLength = function () {
	this.length = this.getLength();
};

Util.Vector.prototype.updateNormalized = function () {
	var length = this.getLength();
	this.normalized =  new Util.Vector(this.x / length, this.y / length);
};

Util.Vector.prototype.updatePerpendicular = function () {
	this.perpendicular = new Util.Vector(-this.y, this.x);
};

/* testers */
Util.Vector.prototype.isGreaterThan = function (other) {
	return (this.x > other.x) && (this.y > other.y);
};

Util.Vector.prototype.isGreaterThanBETA = function (other) {
	return Math.abs(this.x) + Math.abs(this.y) > Math.abs(other.x) + Math.abs(other.y);
};

Util.Vector.prototype.isEqualsTo = function (other) {
	return (this.x == other.x) && (this.y == other.y);
};
...
Util.Polygon.prototype.setEdges = function (edges) {
	...
	for (var i in this.edges) {
		...
		this.edges[i].updateNormalized();
		this.edges[i].normalized.updatePerpendicular();
	}

	// liga o último ponto com o primeiro
	this.edges.push(this.points[this.points.length - 1].getPathTo(this.position));
	// calcula o vetor normalizado e perpendicular
	this.edges[this.edges.length - 1].updateNormalized();
	this.edges[this.edges.length - 1].normalized.updatePerpendicular();
};
...
Util.Polygon.prototype.isIntersecting = function (form) {
	...
	if (form instanceof Util.Circle) {
		var shortestPathToCircle = this.position.getPathTo(form.position), path;

		for (var i in this.points) {
			path = this.points[i].getPathTo(form.position);
			if (shortestPathToCircle.isGreaterThanBETA(path)) {
				shortestPathToCircle = path;
			}
		}
		var axis = shortestPathToCircle.getNormalized();
		...
		edges = this.edges.concat(form.edges);
	}
...
Util.Polygon.getRegular = function (context, centerPosition, numberOfSides, size, color) {
	...
	for (var i = 1, lastPosition = new Util.Vector(size, 0); i < numberOfSides; i++) {
		...
		edges.push(lastPosition.getPathTo(points[i - 1]));
...
Util.Circle.prototype.getProjectionOnto = function (axis) {
	var projection = this.position.getDotProduct(axis);
	return new Util.Projection(projection - this.radius, projection + this.radius);
};

Util.Circle.prototype.isIntersecting = function (form) {
	if (form instanceof Util.Polygon) {
		return form.isIntersecting(this);
	}
	return this.position.getPathTo(form.position).getLength() < this.radius + form.radius;
};
